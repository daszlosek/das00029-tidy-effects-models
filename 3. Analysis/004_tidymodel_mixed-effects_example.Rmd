...
First lets load our packages of interest. For the ```multilevelmod``` package you will need to install it through github using ```devtools::install_github()```. We will also be loading in one of R's most used mixed effects modeling packages,```lme4```, some data.
```{r}
...
Now lets load in the sleep study dataset from the ``lme4``` package. I am also going to create a fake categorical variable to use as an example 
